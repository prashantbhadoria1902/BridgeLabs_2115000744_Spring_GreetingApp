...
public class GreetingController {
    ...
    public ResponseEntity<Greeting> updateGreeting(@PathVariable Long id, @RequestBody Greeting updatedGreeting) {
        Optional<Greeting> existingGreeting = greetingService.findGreetingById(id);

        if (existingGreeting.isPresent()) {
            Greeting greeting = existingGreeting.get();
            greeting.setFirstName(updatedGreeting.getFirstName());
            greeting.setLastName(updatedGreeting.getLastName());
